...
2. ВАЖНО: если в описании указан размер порции (кусочек, половина, полпорции) - используй его для расчета!
   - Кусочек = примерно 1/8 от стандартной порции
   - Половина/полпорции = 50% от стандартной порции
   - Стандартная порция = ~200-300г
3. Оцени порции (граммы) с учетом указанного размера
4. Рассчитай калории и БЖУ для КАЖДОГО продукта
5. Суммируй итого
...
- Кусочек торта (не целый торт!): ~320 ккал, 4г белка, 15г жира, 42г углеводов
